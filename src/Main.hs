...
readProc [Port port] =
    (liftIO $ hGetLine port) >>= liftThrows . readExpr

writeProc :: [LispVal] -> IOThrowsError LispVal
writeProc [obj] = writeProc [obj, Port stdout]
writeProc [obj, Port port] =
    liftIO $ hPrint port obj >> (return $ Bool True)
...
makeFunc varargs env params body = return $ Func (map showVal
    params) varargs body env
...
runOne args = do
    ...
    (runIOThrows $ liftM show $ eval env
        (List [Atom "load", String (args !! 0)])) >>= hPutStrLn stderr
